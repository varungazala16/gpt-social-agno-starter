...
async def list_connections(
    ...
    result = await db.execute(select(SocialAccount).where(SocialAccount.user_id == UUID(current_user.id)))
    accounts = result.scalars().all()

    return SocialAccountListResponse(
        accounts=[SocialAccountResponse.model_validate(account) for account in accounts], total=len(accounts)
    )
